...
Run utility benchmarks (MMLU / TruthfulQA via evaluate_benchmark.py, same as
run_evaluate_benchmark.sh) across every checkpoint listed in a
generate_perturbed_checkpoints.py manifest, plus (optionally) the original
un-noised poisoned checkpoint as a baseline, and compile everything into ONE
csv: checkpoint name + mmlu + truthfulqa (plus enough metadata to slice by
noise_mode/std/etc.). Mirrors run_asr_batch.py exactly, just for utility
instead of ASR.

Pure eval batch driver -- no model is loaded in THIS process. Each
checkpoint's eval is a separate `python evaluate_benchmark.py ...` subprocess
(via quant_specific.noise_violation.run_benchmark_eval, the same function
...
recorded as NaN mmlu/truthfulqa with the returncode/stderr tail, and the
batch continues.
...
Example:
    ...
    python run_benchmark_batch.py \\
        ...
        --baseline_model_name_key qwen2.5-3b-instruct \\
        --output_dir benchmark_batch_results/qwen2.5-3b-instruct-ad_inject-nf4
"""
...
from quant_specific.noise_violation import cgroup_mem_mb, extract_primary_metric, run_benchmark_eval
...
def main():
    ...
    for _, row in manifest.iterrows():
        jobs.append({
            ...
            "model_name_key": row["model_name_key"],
        })
    ...
    if args.baseline_checkpoint:
        ...
        baseline_key = args.baseline_model_name_key or (distinct_key[0] if len(distinct_key) == 1 else None)
        if baseline_p_type is None or baseline_quant is None or baseline_key is None:
            raise SystemExit(
                "Manifest has multiple p_type/quantize_method/model_name_key values -- pass "
                "--baseline_p_type, --baseline_quantize_method, and --baseline_model_name_key "
                "explicitly to know how to evaluate --baseline_checkpoint."
            )
        jobs.insert(0, {
    ...
    for i, job in enumerate(jobs):
        ...
        print(f"\n[{i + 1}/{len(jobs)}] Benchmarking {job['checkpoint_name']} "
        ...
        try:
            scores, bench_stdout, bench_stderr, rc = run_benchmark_eval(
                acl_dir=args.acl_dir,
                model_dir=job["checkpoint_dir"],
                output_dir=eval_output_dir,
                model_name_key=job["model_name_key"],
            ...
            mmlu = extract_primary_metric(scores, "mmlu")
            truthfulqa = extract_primary_metric(scores, "truthfulqa")
        except Exception as e:
            print(f"WARNING: eval crashed for {job['checkpoint_name']}: {e}")
            scores, mmlu, truthfulqa, bench_stderr, rc = {}, float("nan"), float("nan"), str(e), -1

        elapsed = time.perf_counter() - t0
        if rc != 0:
            print(f"WARNING: eval subprocess exited {rc} for {job['checkpoint_name']}. stderr tail:\n{bench_stderr}")
        print(f"MMLU={mmlu}, TruthfulQA={truthfulqa} ({elapsed:.1f}s)")
        ...
        cols = ["checkpoint_name", "mmlu", "truthfulqa", "noise_mode", "std", "p_type", "quantize_method",
                "model_name_key", "checkpoint_dir", "eval_seconds", "eval_returncode"]
        df = df[cols]
        df.to_csv(os.path.join(args.output_dir, "benchmark_results.csv"), index=False)

    total = time.perf_counter() - script_t0
    print(f"\nDone: {len(results)} results in {os.path.join(args.output_dir, 'benchmark_results.csv')} ({total / 60:.1f} min total)")
